...
class Move:
    def __init__(self, type_of_move, figure, row, col, taken_figure=None):
        ...
        self.row = row  # строка, куда ходит фигура. Важно! При ходе - конверсии (превращении пешки) на поле с этой коородинатой встанет ферзь
        self.col = col   # столбец, куда ходит фигура. Важно! При ходе - конверсии (превращении пешки) на поле с этой коородинатой встанет ферзь
        ...
        self.basic_taken_figure_row = self.taken_figure.row if self.taken_figure else None  # строка, с которой съедается фигура
        self.basic_taken_figure_col = self.taken_figure.col if self.taken_figure else None  # столбец, с которой съедается фигура
    ...
    def apply_move(self, board):
        ...
        if self.type_of_move == NORMAL_MOVE:
            ...
            board.board[self.row][self.col] = self.figure  # поставили фигуру на место съединой фигуры
            ...
            queen = board.create_queen_for_pawn(self.row, self.col, self.figure.color)  # создаем ферзя , который займет место пешки на row, col
            ...
            if self.taken_figure is not None:  # если ход со взятием, то
                board.all_figures[self.taken_figure.color].remove(self.taken_figure)  # съеденная фигура убрана из списка фигур
            ...
            board.board[self.basic_row][self.basic_col] = None  # убрали фигуру, которая съела с прежнего места
            board.board[self.basic_taken_figure_row][self.basic_taken_figure_col] = None  # убираем фигуру, которая была съедена на проходе
            board.all_figures[self.taken_figure.color].remove(self.taken_figure)  # съеденная фигура убрана из списка фигур
            ...
            if self.taken_figure.col == 0:  # если ладья была на 0-м столбце, то
                board.board[self.row][self.taken_figure.col + 3] = self.taken_figure  # поставили ладью на 3 столбец (ВАЖНО!!! БЕРЕМ COL ЛАДЬИ (taken_figure), а не КОРОЛЯ!!!)
                self.taken_figure.col = self.taken_figure.col + 3  # ладья стала на 3 столбец (ВАЖНО!!! БЕРЕМ COL ЛАДЬИ (taken_figure), а не КОРОЛЯ!!!)
    ...
    def undo_move(self, board):
        ...
        if self.type_of_move == NORMAL_MOVE:
            ...
            self.figure.row = self.basic_row  # фигура вернулась на место, с которого она ходила
            self.figure.col = self.basic_col  # фигура вернулась на место, с которого она ходила
            board.all_figures[self.taken_figure.color].append(self.taken_figure)  # съеденная фигура вернулась в список фигур
        elif self.type_of_move == CONVERSION_MOVE:
            ...
            board.board[self.basic_taken_figure_row][self.basic_taken_figure_col] = self.taken_figure  # ставим съеденную фигуру на место, где она была
            board.board[self.basic_row][self.basic_col] = self.figure  # ставим фигуру на место, с которого она ходила
            self.figure.row = self.basic_row  # фигура вернулась на место, с которого она ходила
            self.figure.col = self.basic_col  # фигура вернулась на место, с которого она ходила
            ...
            board.board[self.basic_row][self.basic_col] = self.figure  # ставим фигуру (КОРОЛЯ!) на место, с которого он ходила
            board.board[self.row][self.col] = None  # убираем фигуру (КОРОЛЯ!) с места, на которое он ходил
            self.figure.row = self.basic_row  # фигура вернулась на место, с которого она ходила
            self.figure.col = self.basic_col  # фигура вернулась на место, с которого она ходила

            board.board[self.basic_taken_figure_row][self.basic_taken_figure_col] = self.taken_figure  # ставим фигуру (ЛАДЬЮ!) на место, с которого она ходила
